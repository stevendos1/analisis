...
namespace Spendnt.WEB.Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<HttpResponseWrapper<T>> GetAsync<T>(string url)//200
        {
        ...
        public async Task<HttpResponseWrapper<TResponse>> PostAsync<T, TResponse>(string url, T model)//200
        {
        ...
        public async Task<HttpResponseWrapper<TResponse>> PutAsync<T, TResponse>(string url, T model)//200
        {
            var messageJSON = JsonSerializer.Serialize(model);
            var messageContet = new StringContent(messageJSON, Encoding.UTF8, "application/json");
            var responseHttp = await _httpClient.PutAsync(url, messageContet);
